...
void showVerseInfoBottomSheet({
  ...
  final quranReadingController = Get.put(QuranReadingController());
  if (quranReadingController.isFullScreenMode.value) {
    quranReadingController.isFullScreenMode.value = false;
    quranReadingController.update();
  }
  verse.isHighlighted.value = false;
  await Get.delete<AyahBottomSheetController>();
  verse.isHighlighted.value = true;
  // word.isHighlighted.value = true;
  if (word.wordType == 'end') {
    word = verse.words[verse.words.indexOf(word) - 1];
