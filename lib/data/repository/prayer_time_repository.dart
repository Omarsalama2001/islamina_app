...
class PrayerTimeRepository extends GetxService {
  ...
  Future<void> showPrayerNotification() async {
    ...
    if (currentPrayer.isNotificationEnabled.value) {
      if (currentPrayer.type == Prayer.sunrise) {
        await Get.find<NotificationService>().showSunriseNotification(
          currentPrayer,
        );
      } else {
  ...
  Future<void> showIqamahPrayerNotification() async {
    ...
    if (currentPrayer.isNotificationEnabled.value) {
      if (currentPrayer.type != Prayer.sunrise) {
        await Get.find<NotificationService>().showIqamahPrayerNotification(
          currentPrayer,
        );
      }
    }
  ...
  Future<void> showBeforePrayerNotification() async {
    ...
    if (currentPrayer.isNotificationEnabled.value) {
      if (currentPrayer.type != Prayer.sunrise) {
        await Get.find<NotificationService>().showBeforePrayerNotification(
          currentPrayer,
        );
      }
    }
...
// Get the Arabic representation (ص or م) based on whether it's AM or PM
  ...
  void increaseCalender() {
    DateTime newDate = now.add(const Duration(days: 1));
    DateTime oneMonthFromNow = DateTime.now().add(const Duration(days: 30));
    if (newDate.isAfter(oneMonthFromNow)) {
      return;
    }
    now = newDate;
    getPrayerTimesByDate(newDate);
  }

  void decreaseCalender() {
    DateTime newDate = now.subtract(const Duration(days: 1));
    if (newDate.isBefore(DateTime.now().subtract(1.days))) {
      return;
    }
    now = newDate;
    getPrayerTimesByDate(newDate);
  ...
  String convertToArabicTime(String time) {
    ...
    List<String> timeParts = period.split(':');
